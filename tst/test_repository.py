...
def test_repository():
    ...
    assert result_errors.shape == (3, 6)
    ...
    assert result_errors_w_max_models.shape == (3, 6)
    ...
    assert repo.evaluate_ensembles(datasets=[dataset], configs=[config, config],
                                  ensemble_size=5, folds=[2], backend="native")[0].shape == (1, 6)
    ...
    assert repo.evaluate_ensembles(datasets=[dataset], configs=[config, config], ensemble_size=5, backend="native")[0].shape == (2, 6)
    assert repo.evaluate_ensembles(datasets=[dataset], configs=[config, config], ensemble_size=5, folds=[2], backend="native")[0].shape == (1, 6)
    assert repo.evaluate_ensemble(dataset=dataset, fold=2, configs=[config, config], ensemble_size=5)[0].shape == (1, 6)
    ...
    assert repo.predict_test(dataset=dataset, config=config, fold=2).shape == (13, 25)
    assert repo.dataset_metadata(dataset=dataset) == {'dataset': dataset, 'task_type': 'TaskType.SUPERVISED_CLASSIFICATION'}
    assert repo.evaluate_ensembles(datasets=[dataset], configs=[config, config], ensemble_size=5, backend="native")[0].shape == (1, 6)
