    ...
    for tid in repo1.tids():
        for c in repo1.get_configs():
            for f in repo1.folds:
                repo1_test = repo1.predict_test_single(tid=tid, config=c, fold=f)
                repo2_test = repo2.predict_test_single(tid=tid, config=c, fold=f)
                repo1_val = repo1.predict_val_single(tid=tid, config=c, fold=f)
                repo2_val = repo2.predict_val_single(tid=tid, config=c, fold=f)
...
def test_repository():
    ...
    assert repo.predict_val_single(tid=tid, config=config_name, fold=2).shape == (123, 25)
    assert repo.predict_test_single(tid=tid, config=config_name, fold=2).shape == (13, 25)
    ...
    result_errors, result_ensemble_weights = repo.evaluate_ensemble(tids=[tid], configs=[config_name, config_name], ensemble_size=5, backend="native")
    ...
    assert repo.evaluate_ensemble(tids=[tid], configs=[config_name, config_name],
    ...
    assert repo.evaluate_ensemble(tids=[tid], configs=[config_name, config_name], ensemble_size=5, backend="native")[0].shape == (1, 2)
    assert repo.evaluate_ensemble(tids=[tid], configs=[config_name, config_name],
                                  ensemble_size=5, folds=[2], backend="native")[0].shape == (1, 1)
    ...
    assert repo.get_configs() == [config_name]
    assert repo.predict_val_single(tid=tid, config=config_name, fold=2).shape == (123, 25)
    assert repo.predict_test_single(tid=tid, config=config_name, fold=2).shape == (13, 25)
    assert repo.dataset_metadata(tid=tid) == {'tid': 359946, 'name': dataset_name, 'task_type': 'TaskType.SUPERVISED_CLASSIFICATION'}
    assert repo.evaluate_ensemble(tids=[tid], configs=[config_name, config_name], ensemble_size=5, backend="native")[0].shape == (1, 1)
