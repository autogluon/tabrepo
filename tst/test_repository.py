...
def test_repository():
    ...
    assert repo.predict_val_single(tid=tid, config=config, fold=2).shape == (123, 25)
    assert repo.predict_test_single(tid=tid, config=config, fold=2).shape == (13, 25)
    assert repo.predict_val(tid=tid, fold=2, configs=[config]).shape == (1, 123, 25)
    assert repo.predict_test(tid=tid, fold=2, configs=[config]).shape == (1, 13, 25)
