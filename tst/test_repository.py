    ...
    for tid in repo1.tids():
        for c in repo1.list_models():
            for f in repo1.folds:
                repo1_test = repo1.predict_test_single(tid=tid, config_name=c, fold=f)
                repo2_test = repo2.predict_test_single(tid=tid, config_name=c, fold=f)
                repo1_val = repo1.predict_val_single(tid=tid, config_name=c, fold=f)
                repo2_val = repo2.predict_val_single(tid=tid, config_name=c, fold=f)
...
def test_repository():
    ...
    assert repo.predict_val_single(tid=tid, config_name=config_name, fold=2).shape == (123, 25)
    assert repo.predict_test_single(tid=tid, config_name=config_name, fold=2).shape == (13, 25)
