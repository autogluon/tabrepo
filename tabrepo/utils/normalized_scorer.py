...
class NormalizedScorer:
    def __init__(self,
        ...
        self.topline_dict = df_results_by_dataset.groupby(dataset_col).min()[metric_error_col].to_dict()
        if baseline is not None:
            ...
            ].groupby(dataset_col).min()[metric_error_col].to_dict()
        else:
            self.baseline_dict = df_results_by_dataset.groupby(dataset_col).median(numeric_only=True)[
