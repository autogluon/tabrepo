...
def get_dataset(dataset_name, dataset_path):
    ...
    N_trainval = (
        None
        if N is None
        else {key: N[key] for key in ["train", "val"]}
        if "train" in N and "val" in N
        else None
    )
    N_test = (
        None
        if N is None
        else {key: N[key] for key in ["test"]}
        if "test" in N
        else None
    )
    ...
    C_trainval = (
        None
        if C is None
        else {key: C[key] for key in ["train", "val"]}
        if "train" in C and "val" in C
        else None
    )
    C_test = (
        None
        if C is None
        else {key: C[key] for key in ["test"]}
        if "test" in C
        else None
    )
...
def num_enc_process(
    N_data, num_policy, n_bins=2, y_train=None, is_regression=False, encoder=None
):
    ...
    """
    from tabrepo.benchmark.models.ag.beta.deps.talent_num_embeddings import (
        BinsEncoding,
        JohnsonEncoding,
    ...
    if C_data is not None:
        ...
        if mode_values is not None:
            ...
            for column_idx in range(C_data["test"].shape[1]):
                C_data["test"][:, column_idx][
                    C_data["test"][:, column_idx] == unknown_value
            ...
            for column_idx in range(C_data["val"].shape[1]):
                C_data["val"][:, column_idx][
                    C_data["val"][:, column_idx] == unknown_value
                ] = mode_values[column_idx]
        ...
        if cat_policy == "ordinal":
            ...
            C_data = {
                k: cat_encoder.transform(v.astype(str)).values
...
def data_loader_process(is_regression, X, Y, y_info, device, batch_size, is_train):
    ...
    if is_train:
        ...
        train_loader = DataLoader(
            dataset=trainset, batch_size=batch_size, shuffle=True, num_workers=0
        )
        val_loader = DataLoader(
            dataset=valset, batch_size=batch_size, shuffle=False, num_workers=0
        )
        return X[0], X[1], Y, train_loader, val_loader, loss_fn
    testset = TData(is_regression, X, Y, y_info, "test")
    test_loader = DataLoader(
        dataset=testset, batch_size=batch_size, shuffle=False, num_workers=0
    )
...
def get_categories(X_cat: dict[str, torch.Tensor] | None) -> list[int] | None:
