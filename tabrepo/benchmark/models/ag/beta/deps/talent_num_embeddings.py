...
class _JohnsonEncodingImpl(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        bin_indices = torch.stack(
            [
                torch.bucketize(x[..., i], self.edges[i], right=True) - 1
        ...
        johnson_code = torch.zeros(
            *x.shape, code_length, device=x.device, dtype=torch.float32
        )
        for i in range(x.shape[0]):
            for j in range(x.shape[1]):
                johnson_code[i, j, :] = self.temp_code(
                    bin_indices[i, j].item(), max_bin
                )
