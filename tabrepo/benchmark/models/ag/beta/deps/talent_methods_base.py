...
class Method(metaclass=abc.ABCMeta):
    ...
    def data_format(self, is_train=True, N=None, C=None, y=None):
        ...
        if is_train:
            ...
            self.N, self.C, self.ord_encoder, self.mode_values, self.cat_encoder = (
                data_enc_process(self.N, self.C, self.args.cat_policy, self.y["train"])
            )
    ...
    def predict(self, data, info, model_name):
        ...
        self.model.load_state_dict(
            torch.load(
                osp.join(self.args.save_path, model_name + f"-{self.args.seed!s}.pth")
            )["params"]
        )
        print(
            "best epoch {}, best val res={:.4f}".format(
    ...
    def metric(self, predictions, labels, y_info):
        ...
        if self.is_binclass:
            ...
            avg_recall = skm.balanced_accuracy_score(
                labels, predictions.argmax(axis=-1)
            )
            avg_precision = skm.precision_score(
                labels, predictions.argmax(axis=-1), average="macro"
            )
            f1_score = skm.f1_score(
                labels, predictions.argmax(axis=-1), average="binary"
            )
        ...
        if self.is_multiclass:
            ...
            auc = skm.roc_auc_score(
                labels, predictions, average="macro", multi_class="ovr"
            )
            return (accuracy, avg_recall, avg_precision, f1_score, log_loss, auc), (
                "Accuracy",
                "Avg_Recall",
