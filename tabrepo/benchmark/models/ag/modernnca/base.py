# from https://github.com/LAMDA-Tabular/TALENT/blob/cb6cb0cc9d69ac75c467e8dae8ca5ac3d3beb2f2/TALENT/model/methods/base.py#L1
...
from tabrepo.benchmark.models.ag.modernnca.utils import Averager, Timer, get_device, set_seeds
...
class Method(metaclass=abc.ABCMeta):
    ...
    def data_format(self, is_train=True, N=None, C=None, y=None):
        ...
        if is_train:
            ...
            self.N, self.num_encoder = num_enc_process(
                self.N,
            ...
            self.N, self.C, self.ord_encoder, self.mode_values, self.cat_encoder = data_enc_process(
                self.N, self.C, self.args.cat_policy, self.y["train"]
            )
            ...
            N_test, C_test, _, _, _ = data_enc_process(
                N_test, C_test, self.args.cat_policy, None, self.ord_encoder, self.mode_values, self.cat_encoder
            )
    ...
    def predict(self, data, info, model_name):
        ...
        self.model.load_state_dict(
            torch.load(osp.join(self.args.save_path, model_name + f"-{self.args.seed!s}.pth"))["params"]
        )
        ...
        with torch.no_grad():
            for _i, (X, y) in tqdm(enumerate(self.test_loader)):
        ...
        for name, res in zip(metric_name, vres, strict=False):
            print(f"[{name}]={res:.4f}")
    ...
    def validate(self, epoch):
        ...
        test_logit, test_label = [], []
        with torch.no_grad():
            for _i, (X, y) in tqdm(enumerate(self.val_loader)):
                if self.N is not None and self.C is not None:
                    X_num, X_cat = X[0], X[1]
                elif self.C is not None and self.N is None:
                    X_num, X_cat = None, X
