...
def data_nan_process(N_data, C_data, num_nan_policy, cat_nan_policy, num_new_value=None, imputer=None,
                     cat_new_value=None):
    """
    ...
    if C_data is None:
        ...
        cat_nan_masks = {k: np.isnan(v) if np.issubdtype(v.dtype, np.number) else np.isin(v, ['nan', 'NaN', '', None])
                         for k, v in C.items()}
...
def num_enc_process(N_data, num_policy, n_bins=2, y_train=None, is_regression=False, encoder=None):
    ...
    """
    from tabrepo.benchmark.models.ag.modernnca.num_embeddings import compute_bins, PiecewiseLinearEncoding, \
        UnaryEncoding, JohnsonEncoding, \
        BinsEncoding
    if N_data is not None:
        if num_policy == 'none':
            ...
            if encoder is None:
                ...
                bins = compute_bins(N_data['train'], n_bins=n_bins, tree_kwargs=tree_kwargs,
                                    y=torch.from_numpy(y_train), regression=is_regression)
                encoder = PiecewiseLinearEncoding(bins)
            for item in N_data:
                N_data[item] = encoder(N_data[item]).cpu().numpy()
